...
@section('content')
<div class="page">
<img style="width: 100%;" src="{{ url('storage/' . $newsInner->image) }}" alt="">
    <div class="container">
        @if(session('success'))
        ...
        @can('view', Auth::user())
            <div class="d-flex justify-content-between mt-2">
                <div class="news__btn-wrp">
                    <a href="{{ route('edit.news', $newsInner->id) }}">
                ...
                <div class="news__btn-wrp">
                    <form action="{{ route('delete.news') }}" method="POST">
                        @csrf
                        @method('delete')
                        <input type="hidden" name="id" value="{{ $newsInner->id }}">
        ...
        @endcan
        <div  class="mt-2 pb-2 text-break">
            <h4 class="inner_title">{{ $newsInner->title }}</h4>
            <h5 style="color: white">{{ $newsInner->subtitle }}</h5>
            <div>
                <p class="text">{!! $newsInner->text !!}</p>
            </div>
        </div>
    </div>
</div>
